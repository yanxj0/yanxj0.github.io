// 结合echarts
function cesiumUseEcharts(viewer, option) {
  //结合echarts
  (function (e) {
    const t = {};
    function n(r) {
   ...
   * @param {*} target
   */
  const insertAfter = (anchorNode, target) => {
    const parentNode = anchorNode.parentNode;
    if (parentNode) {
      if (parentNode.childNodes?.length === 1) {
        parentNode.appendChild(target);
      } else {
        parentNode.insertBefore(target, anchorNode.nextSibling);
      }
    }
  };
